...
def _ensure_fts_index(
    ...
    *,
    ...
    if not _validate_fts_index(execute_sql):
        raise RuntimeError("DuckDB FTS index rebuild completed but BM25 macro is still unavailable")
        ...
        if query.needle:
            try:
                await self.conn.execute(
                    FTS_VALIDATE_SQL,
                    ["__knowlt_fts_probe__", query.needle],
                )
            except Exception as exc:
                if not _is_missing_fts_macro_error(exc):
                    raise
                await self.data_repo.refresh_indexes()
